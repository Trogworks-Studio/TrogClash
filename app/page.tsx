  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-8 px-4 py-10">
      <div className="flex flex-col items-center gap-2">
        <div className="h-28 w-28 animate-float-slow overflow-hidden rounded-full border-4 border-goblin-gold bg-swamp-800 shadow-panel">
          <GoblinIllustration variant="chief" rarity="legendary" className="h-full w-full" />
        </div>
        <h1 className="font-display text-4xl font-extrabold tracking-tight text-parchment-100 toon-outline">
          Trog <span className="text-ember-600">Clash</span>
        </h1>
        <p className="font-display text-xs uppercase tracking-[0.3em] text-goblin-gold">Trogworks Studyo</p>
      </div>

      <div className="w-full max-w-sm rounded-2xl border-2 border-swamp-700 bg-swamp-900/80 p-4 shadow-panel">
        <div className="flex items-center justify-between">
          {editingName ? (
            ...
            >
              {profile.loading ? "Yükleniyor…" : profile.username} ✏️
            </button>
          )}
          <span className="rounded-full bg-goblin-gold px-3 py-1 font-display text-xs font-bold text-swamp-950">
        ...
        <div className="mt-3 h-3 w-full overflow-hidden rounded-full bg-swamp-950">
          <div className="h-full rounded-full bg-moss-500 transition-all" style={{ width: `${xpPct}%` }} />
        </div>
      ...
      <div className="flex flex-col gap-3">
        <Link
          href="/play"
          className="rounded-full border-2 border-swamp-950 bg-ember-600 px-10 py-4 text-center font-display text-lg font-extrabold text-white shadow-card transition-transform hover:scale-105 active:scale-95"
        >
          🪓 Bot ile Kapış
        </Link>
        <Link
          href="/leaderboard"
          className="rounded-full border-2 border-swamp-950 bg-swamp-800 px-10 py-3 text-center font-display text-sm font-bold text-parchment-100 shadow-card transition-transform hover:scale-105 active:scale-95"
        >
          🏆 Skor Tablosu
        </Link>
      </div>
